  ...
  const ExchangeCard = ({ exchange, index }: { exchange: any; index: number }) => (
    ...
    >
      <div className="w-full md:w-4/12 aspect-[16/8.5] relative overflow-hidden bg-zinc-100">
        <a href={exchange.link} className="block w-full h-full">
          <img
            src={exchange.image}
            alt={exchange.title}
      ...
      <div className="w-full md:w-8/12 flex flex-col justify-center px-6">
        <h2 className="text-xl md:text-2xl font-bold mb-2 group-hover:text-zinc-600 transition-colors tracking-tight">
          <a href={exchange.link} className="text-zinc-900 hover:text-zinc-600 transition-colors">
            {language === "en" ? exchange.titleEn : exchange.title}
          </a>
        </h2>

        <p className="text-sm text-zinc-500 mb-5 font-light">
          <a href={exchange.link} className="text-zinc-500 hover:text-zinc-900 transition-colors">
            {language === "en" ? exchange.dateEn : exchange.date}
          </a>
        </p>

        <p className="text-sm text-zinc-600 mb-5 font-light">
          <a href={exchange.link} className="text-zinc-600 hover:text-zinc-900 transition-colors">
